...
public class TesteRepository implements Interface_Teste {
    ...
    private final static String ALTERAR_MODELO = "UPDATE taiff.testes SET nome_modelo=? WHERE id_teste=?";
    ...
    public Teste inserirTeste(Teste teste) {
        ...
        try {
            ...
            teste.setId_teste(holder.getKey().intValue());
            teste.getZeropeca().setId_zeropeca(chaveZeroPeca);
    ...
    public Teste inserirTesteEZeroPeca(Teste teste) {
    ...
    public int alterarNomeModelo(int id, Teste teste) {
        try {
            teste.setId_teste(id);
            Object[] params = {teste.getNome_modelo(), id};
            return jdbcTemplate.update(ALTERAR_MODELO, params);
        } catch (Exception e) {
            return -1;
        }
    }
